...
with open(objectName + ".obj") as objFile:
...
for triangleIndex in range(fLen):
    ...
    for trianglePartIndex in range(trianglePartsLen):
        newTriangle += str(int(triangleParts[trianglePartIndex]) - 1)# - maxi + 1) # - 1
...
with open(objectName + ".struc", 'w') as strucFile:
