...
class User
{
    public:
        ...
        bool isPlayerInTarget(/*std::string*/User*, double = 99999)/*could use DBL_MIN or DBL_MAX*/, isCreative(), isNPC(), m_needRefresh = false, m_refreshOBB = false, isBAC(), isVanished(), hasLicense(unsigned short), getLicenses(), m_licenses[LICENSE_NUMBER], isCuffed(), isTasered(), isEscorted(),
             isEscorting(), isRidingVehicle(), isDriver(), hasKey(unsigned int), getNeedHandRender(), getNeedBackRender(), getStuffInHands(), getStuffOnBack(), getJustRemovedHand(), getJustRemovedBack(), getNeedToBeRemoved(), getOBBVBOCreated(), isFlying(), cantMove(); // getLicenses() to test/remove
