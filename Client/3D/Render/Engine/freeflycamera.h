...
class FreeFlyCamera
{
    public:
        ...
        virtual void onMouseMotion(const SDL_MouseMotionEvent &event, bool = false, double = 0, double = 0),
                     onKeyboard(const SDL_KeyboardEvent &event), animate(Uint32 timestep), setPosition(const Vector3D &position),
                     setPosition(double),  look(), vectorsFromAngles(bool = true, bool force = false), addTheta(double, bool clamp = false), addPhi(double)/*, doCuffJob()*/, addRoll(double), setRoll(double), setPositionCommon()/*, setPosition(Position)*/, checkChunk(); // don't save roll (on server side because it is a temporary parameter ?) and for vehicles ?
...
void changeF5Bool(), changeF7Bool(), setF7Bool(bool, bool = false), changeF5Distance(double F5Distance = DEFAULT_F5_DISTANCE), setAllInteractionEnabled(bool = false); // true could be a default case for setAllInteractionEnabled (like resetting earlier status or just use default argument as the default type value)
