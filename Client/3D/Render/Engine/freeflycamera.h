...
void changeF5Bool(), changeF7Bool(), setF5Bool(bool = false), setF7Bool(bool, bool = false), changeF5Distance(double F5Distance = DEFAULT_F5_DISTANCE), setAllInteractionEnabled(bool = false), forceF5(); // true could be a default case for setAllInteractionEnabled (like resetting earlier status or just use default argument as the default type value)
