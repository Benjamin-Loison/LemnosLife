...
/*#ifndef _WIN64
    ...
    extern "C"
    {
            ...
            static int encode_base64(char *, const u_int8_t *, size_t), decode_base64(u_int8_t *, size_t, const char *), bcrypt_initsalt(int log_rounds, uint8_t *salt, size_t saltbuflen);
...
static int bcrypt_initsalt(int log_rounds, uint8_t *salt, size_t saltbuflen)
{
	...
	srand(time(NULL)); // not generating the same number ?
	for(unsigned short csaltIndex = 0; csaltIndex < BCRYPT_MAXSALT; csaltIndex++)
        csalt[csaltIndex] = rand() % 256;
	...
	if(log_rounds < 4)
		...
		log_rounds = 31;
	uint8_t *hdrstr = (uint8_t*)"$2b$%2.2u$";
	snprintf((char*)salt, saltbuflen, (const char*)hdrstr, log_rounds);
...
static int bcrypt_hashpass(const char *key, const char *salt, char *encrypted, size_t encryptedlen)
{
	...
	u_int8_t ciphertext[4 * BCRYPT_WORDS + 1] = "OrpheanBeholderScryDoubt"; //+1 string overflows array bsd knows
...
string bcrypt_checkpass(string pass, const char *goodhash)
{
    char hash[BCRYPT_HASHSPACE];
	bcrypt_hashpass(pass.c_str(), goodhash, hash, sizeof(hash));
	string hashStr = string(hash);
	explicit_bzero(hash, BCRYPT_HASHSPACE);
	return hashStr;
}
...
{
	u_int8_t *bp = buffer;
	//const u_int8_t *p = b64data; //c++ bitch
