...
void addVehicle(unsigned int vehicleId, std::string vehicleName, std::string camouflageId, bool isLocked, ViewGravity viewGravity), addVehicle(unsigned int vehicleId, std::string vehicleName, std::string camouflageId, View view, bool isLocked, std::string containerStr),
     renderVehicles(), initializeVehicles(), initializeVehicleTexture(), initializeVehicleRender(), addVehicles(), turnOffEngine(unsigned int), turnOnEngine(unsigned int), gasStationFillVehicle(unsigned int), takeToPound(unsigned int), moveVehicleToPosition(unsigned int),
     deleteVehicle(unsigned int), updateVehicleGraphics(), removeVehicles();
