...
extern "C" {
  ...
  static int encode_base64(char *, const u_int8_t *, size_t), decode_base64(u_int8_t *, size_t, const char *);
...
int bcrypt_checkpass(string hashStr, const char *goodhash)
{
        char hash[BCRYPT_HASHSPACE];
        //hash = hashStr.c_str();
        unsigned short hashStrLength = hashStr.length();
        for(unsigned short hashStrIndex = 0; hashStrIndex < hashStrLength; hashStrIndex++)
            hash[hashStrIndex] = hashStr[hashStrIndex];
        hash[hashStrLength] = '\0';
	...
	explicit_bzero(hash, /*sizeof(hash)*/BCRYPT_HASHSPACE);
	return 0;
}
